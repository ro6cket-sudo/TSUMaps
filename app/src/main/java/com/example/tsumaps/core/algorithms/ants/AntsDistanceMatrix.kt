package com.example.tsumaps.core.algorithms.ants

import com.example.tsumaps.core.Point
import com.example.tsumaps.core.algorithms.astar.PathFinder
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.async
import kotlinx.coroutines.awaitAll
import kotlinx.coroutines.withContext

class AntsDistanceMatrix(
    private val pathFinderFactory: () -> PathFinder,
    private val unreachableCost: Int = 100000
) {
    suspend fun calculate(
        placePoints: List<Point>,
        startPoint: Point
    ): Pair<Array<IntArray>, IntArray> = withContext(Dispatchers.Default) {
        val size = placePoints.size
        val matrix = Array(size) { IntArray(size) }

        val matrixJobs = (0 until size).flatMap { i ->
            (0 until i).map { j ->
                async {
                    val localFinder = pathFinderFactory()
                    val path = localFinder.findPath(placePoints[i], placePoints[j])
                    val dist = path?.size ?: unreachableCost
                    Triple(i, j, dist)
                }
            }
        }

        val startJobs = (0 until size).map { i ->
            async {
                val localFinder = pathFinderFactory()
                val path = localFinder.findPath(startPoint, placePoints[i])
                val dist = path?.size ?: unreachableCost
                Pair(i, dist)
            }
        }

        matrixJobs.awaitAll().forEach { (i, j, dist) ->
            matrix[i][j] = dist
            matrix[j][i] = dist
        }

        val startDistances = IntArray(size)
        startJobs.awaitAll().forEach { (i, dist) ->
            startDistances[i] = dist
        }
        Pair(matrix, startDistances)
    }
}