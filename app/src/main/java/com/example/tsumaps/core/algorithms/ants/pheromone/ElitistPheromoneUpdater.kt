...
class ElitistPheromoneUpdater(
    private val elitistWeight: Double = 2.0
) : PheromoneUpdater {
    override fun update(
        ...
        if (bestCost in 1 until Int.MAX_VALUE && bestRoute.isNotEmpty()) {
            val bonus = elitistWeight * pheromoneDeposit / bestCost
            var prev = startIdx
            for (idx in bestRoute) {
                pheromone[prev][idx] += bonus
                pheromone[idx][prev] += bonus
                prev = idx
            }
            pheromone[prev][startIdx] += bonus
            pheromone[startIdx][prev] += bonus
        }
    }
}